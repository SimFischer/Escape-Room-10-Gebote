...
(function () {
  ...
  function buildRoomScene(box,opts){
    ...
    function renderGrid(){grid.innerHTML='';opts.objects.forEach(function(o){var tile=buildHotspot(o.icon,o.label,solved[o.id]?'done':'open');tile.addEventListener('click',function(){if(!solved[o.id])openObject(o);});tile.dataset.station=o.id;grid.appendChild(tile);});var book=buildHotspot('📖','Das alte Buch','info');book.addEventListener('click',function(){showDetail(function(c){c.appendChild(el('div','hilfe-panel',opts.hilfe));});});book.dataset.station='buch';grid.prepend(book);var ready=opts.objects.every(function(o){return solved[o.id];});var exit=buildHotspot('🚪','Der Ausgang',ready?'exit-ready':'locked');exit.dataset.station='ausgang';exit.addEventListener('click',function(){if(ready)opts.onComplete();else shake(exit);});grid.appendChild(exit);}
    ...
    function openObject(o){showDetail(function(c){
      c.dataset.station=o.id;
      var tools=el('div','station-tools');var help=el('button','btn book-quick','📖 Im alten Buch nachschlagen');help.type='button';help.addEventListener('click',function(){openBook(opts.hilfe,help);});tools.appendChild(help);c.appendChild(tools);
      if(!['bergpruefung','sorgenweg'].includes(o.id)){var art=el('div','station-art');art.setAttribute('role','img');art.setAttribute('aria-label',o.label+' – illustrierte Abenteuerszene');c.appendChild(art);}
      if(!['bergpruefung','sorgenweg','bruecke','werkstatt'].includes(o.id))c.appendChild(el('h2','station-title',o.label));c.appendChild(el('p','object-flavor',o.flavor));runObjectSteps(c,o.steps,function(){solved[o.id]=true;saveState();refreshWindow();closeDetail();});});}
    renderGrid();
  }
  function openBook(content,trigger){
    var dialog=el('dialog','book-dialog');dialog.setAttribute('aria-label','Das alte Buch');
    var close=el('button','btn book-close','Buch schließen ×');close.type='button';dialog.appendChild(close);dialog.appendChild(el('h2','','Das alte Buch'));dialog.appendChild(el('div','hilfe-panel',content));document.body.appendChild(dialog);
    var pause=document.querySelector('.mountain-toolbar button');var resume=pause&&!pause.disabled&&pause.textContent==='Pause ⏸';if(resume)pause.click();
    close.addEventListener('click',function(){dialog.close();});dialog.addEventListener('close',function(){dialog.remove();if(resume&&pause.isConnected&&pause.textContent==='Weiter ▶')pause.click();if(trigger.isConnected)trigger.focus({preventScroll:true});});dialog.showModal();close.focus();
  }
  ...
  var lastCorrectPosition=-1;
  function renderMCStep(container,step,onSolved){container.appendChild(el('p','question-text',step.prompt()));var hint=makeHint(),opts=el('div','options');var order=shuffle(step.options.map(function(text,i){return {text:text,i:i};}));var pos=order.findIndex(function(x){return x.i===step.correct;});if(pos===lastCorrectPosition&&order.length>1){var other=(pos+1)%order.length,tmp=order[pos];order[pos]=order[other];order[other]=tmp;}order.forEach(function(item,position){var text=item.text,i=item.i;var b=el('button','opt-btn',text);b.addEventListener('click',function(){if(i===step.correct){lastCorrectPosition=position;b.classList.add('correct');opts.querySelectorAll('button').forEach(function(x){x.disabled=true;});hint.textContent='';onSolved();}else{b.classList.add('wrong');shake(b);hint.textContent=step.help;later(function(){b.classList.remove('wrong');},500);}});opts.appendChild(b);});container.appendChild(opts);container.appendChild(hint);}
